...
1. Start with a `GET` call to retrieve the data for your season's games. Export this data into a JSON file, update the `finalScore` fields that are ready to be published, and send the entire JSON payload back to the API as outlined in the next step.
...
4. Our API does not dynamically update fields, so if you want to update your `winLossRatio` to coincide with the recently completed game, you can do so using our [Update Win/Loss Ratio](tut-update-winloss.md)
